...
def findClosestVehicleAhead(vehicle1, vehicles):
  ...
  for vehicle2 in vehicles:
    ...
    diff = theta2 - theta1 if theta2 > theta1 else theta2 + 2*math.pi - theta1
...
def updateAccels(vehicles):
  ...
  for vehicle1 in vehicles:
    ...
    if closest_vehicle:
      dist = vehicle1.getRadius() * diff
      lin_speed1 = toLinear(vehicle1.getAngSpeed(), vehicle1.getRadius())
      lin_speed2 = toLinear(closest_vehicle.getAngSpeed(), closest_vehicle.getRadius())
